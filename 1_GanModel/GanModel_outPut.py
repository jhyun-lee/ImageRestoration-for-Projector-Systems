...
def generator_inference_all(generator_weight_dir,  # 디렉토리 경로 받음
    ...
    for model_num in model_nums:
        weight_path = os.path.join(generator_weight_dir, f"generator_epoch_{model_num}.h5")
        ...
        for idx, (input_dict, _) in enumerate(test_dataset):
            output = generator([input_dict["image_input"], input_dict["color_input"]], training=False)
...
ModelName = "GanModel_PT"

generator_inference_all(
    generator_weight_dir=f"1_GanModel/{ModelName}",
    test_list=InfoAllList,
    save_dir=os.path.join(data_base_dir, fr"GeneratorOutputs\Generator_0806\{ModelName}"),
    Model_Num=50
)
